...
const DepartmentModel = {
  save: async (departmentForm: DepartmentForm) => {
    const query = `insert into department (name, description) values ($1, $2);`;
  ...
  fetch: async () => {
    const query = `select d.id, d.name, d.description from department d where d.is_deleted = false order by id asc;`;
  ...
  getInformation: async (
    ...
    const query = `select d.id, d.name, d.description from department d where d.is_deleted = false and d.id = $1;`;
