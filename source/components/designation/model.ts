...
const DesignationModel = {
  save: async (designationForm: DesignationForm) => {
    const query = `insert into designation (name, description, daily_salary) values ($1, $2, $3);`;
  ...
  fetch: async () => {
    const query = `select d.id, d.name, d.description, d.daily_salary "dailySalary" from designation d where d.is_deleted = false order by id asc;`;
  ...
  getInformation: async (
    ...
    const query = `select d.id, d.name, d.description, d.daily_salary "dailySalary" from designation d where d.is_deleted = false and d.id = $1;`;
  ...
  update: async (designationID: number, designationForm: DesignationForm) => {
    const query = `update designation set name = $1, description = $2, daily_salary = $3 where id = $4;`;
    const { name, description, dailySalary } = designationForm;
  ...
  remove: async (designationID: number) => {
    const query = `update designation set is_deleted = true, deleted_at = now() where id = $1;`;
    const parameters = [designationID];
    await Database.execute(query, parameters);
  },
};
